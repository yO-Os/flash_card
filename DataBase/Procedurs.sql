...
BEGIN
	IF(EXISTS(SELECT 1 FROM  `User` WHERE  `Name`=name_))
    THEN
		set result=0;
	ELSEIF EXISTS(SELECT 1 FROM  `User` WHERE  `Email`=email)  
    THEN
		set result=1;
    ELSE
...
BEGIN
	UPDATE  `Deck`SET `Title`=title,`Description`=description_ WHERE `id`=id;
    IF(row_count()=0)
    THEN
		SET result=0;
	ELSE 
